...
export default function DraggableItem({ item, onDropOnBuddy, onDropOnItem, onDropOnPot, disabled, position }) {
  ...
  const handlePointerMove = (e) => {
    ...
    ;[buddyTarget, potTarget].forEach((target) => {
      ...
      target.classList.toggle('drag-over', inside)
    })
  }
  ...
  const handlePointerUp = (e) => {
    ...
    if (!handled) {
      ...
      if (potTarget && onDropOnPot) {
        ...
        const insidePot = e.clientX >= rect.left && e.clientX <= rect.right && e.clientY >= rect.top && e.clientY <= rect.bottom
        if (insidePot) {
          potTarget.classList.remove('drag-over')
          handled = onDropOnPot(item.id)
        }
      }
    }
    ...
    if (!handled) {
      ...
      if (targetItemId && onDropOnItem) {
        handled = onDropOnItem(item.id, targetItemId)
      }
    }
  ...
  return (
    ...
    >
      {item.image ? (
        ...
        <div className={`food-image food-image-placeholder ${isDragging ? 'dragging' : ''} ${isBouncing ? 'bounce-back' : ''}`}>
          {item.label}
        </div>
      )}
      <div className="food-card-copy">
        <span className="food-card-word">{item.label}</span>
        {item.meaning ? <span className="food-card-meaning">{item.meaning}</span> : null}
      </div>
    </div>
  )
}
