    ...
    <div className="home-root app-shell">
      ...
      <main className="chat-main" style={{ overflowY: 'auto', flex: 1, paddingBottom: '80px' }}>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-10 lg:px-16 py-6 sm:py-8">
          ...
          <SectionHeader
            title="🔔 Thông Báo"
            subtitle="Cập nhật những thành tích xuất sắc, chuỗi ngày học tập và phần thưởng của bé."
            action={
              unreadCount > 0 && (
                <button
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="flex-1">
              ...
              {isLoading ? (
                <div className="flex flex-col items-center justify-center py-20 gap-4">
                  ...
                  <p className="font-mono text-sm text-cream/40 uppercase">Đang tải thông báo...</p>
                </div>
              ) : filtered.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 gap-4">
                  <span className="text-5xl animate-float">🔕</span>
                  <p className="font-grotesk text-xl text-cream/70 uppercase">
                    {filter === 'unread' ? 'Bé đã đọc hết thông báo rồi!' : 'Chưa có thông báo nào'}
                  </p>
                  <p className="font-mono text-sm text-cream/40">
                    {filter === 'unread'
                      ? 'Tất cả thông báo đã được xem.'
                      : 'Bắt đầu học tập để nhận các thành tích và phần thưởng hấp dẫn nhé!'}
                  </p>
                </div>
              ) : (
            ...
            <div className="w-full lg:w-72 space-y-6">
              ...
              <GlassCard className="p-5">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="font-mono text-xs text-cream/50 uppercase">Tổng số</span>
                    <span className="font-grotesk text-lg font-bold text-cream">{notifications.length}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="font-mono text-xs text-cream/50 uppercase">Chưa đọc</span>
                    <span className="font-grotesk text-lg font-bold text-danger">{unreadCount}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="font-mono text-xs text-cream/50 uppercase">Đã đọc</span>
              ...
              <GlassCard className="p-5">
                ...
                <div className="space-y-3">
                  {[
                    { icon: '🏆', title: 'Thành tích', desc: 'Hoàn thành bài học để nhận huy hiệu' },
                    { icon: '🔥', title: 'Chuỗi ngày', desc: 'Học mỗi ngày để giữ chuỗi ngọn lửa' },
                    { icon: '🎁', title: 'Phần thưởng', desc: 'Tích lũy XP và xu từ các bài học' },
                    { icon: '🎯', title: 'Nhiệm vụ', desc: 'Hoàn thành nhiệm vụ ngày để nhận thưởng' },
                  ].map((tip) => (
